...
export const UpdatePageReserva = () => {
    ...
    const getReserva = async () => {
        try {
            const { data } = await axios(`http://localhost:3000/reserva/getId/${id}`, { headers: headers })
    ...
    const updateReserva = async ()=>{
        try {
            ...
            const  { data } = await axios.put(`http://localhost:3000/reserva/update/${id}`, updatedReservation, { headers: headers } )
            console.log(data)
            getHotels()
        } catch (err) {
            console.log(err)
