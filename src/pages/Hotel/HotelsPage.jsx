...
export const HotelsPage = () => {
    ...
    const getAdminHotel = async () => {
        try {
            const { data } = await axios('http://localhost:3000/user/getManager', { headers: headers })
            if (data.users) {
                console.log(data)
                setUser(data.users)
            }
    ...
    const getHotels = async () => {
        try {
            const { data } = await axios('http://localhost:3000/hotel/getHotels', { headers: headers })
            if (data.hotels) {
    ...
    const deleteHotel = async (_id) => {
        try {
            ...
            if (confirmDelete) {
                const { data } = await axios.delete(`http://localhost:3000/hotel/deleteHotel/${_id}`, { headers: headers })
                getHotels()
                alert(`${data.message}`)
            }
    ...
    const search = async () => {
        try {
            ...
            const { data } = await axios.post('http://localhost:3000/hotel/search', getSearch, { headers: headers })
